...
def create_app():
        ...
        if 'user' in session:
            ...
            if user_id:
                try:
                    ...
                    verifier = get_supabase_admin() or supabase
                    res = verifier.table('users').select('id').eq('id', user_id).execute()
                    if not res.data:
    ...
    def reverse_geocode():
        ...
        headers = {
            "User-Agent": "ACCIO HUB-Attendance/1.0 (contact@acciohub.com)"
        }

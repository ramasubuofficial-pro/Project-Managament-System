...
from utils import supabase as anon_supabase, get_supabase_admin

admin_supabase = get_supabase_admin()
supabase = admin_supabase if admin_supabase else anon_supabase
import uuid
import csv
import io
import warnings
...
def get_team():
    try:
        ...
        res = client.table('users').select('*').eq('workspace_id', workspace_id).execute()
        return jsonify(res.data)
    except Exception as e:
        return jsonify({"error": str(e)}), 400

@api_bp.route('/team/<user_id>', methods=['PUT'])
def update_team_member(user_id):
    ...
    try:
        ...
        current_user = client.table('users').select('role').eq('id', current_user_id).execute()
        if not current_user.data or current_user.data[0].get('role') != 'Admin':
            return jsonify({"error": "Forbidden: Only admins can edit roles"}), 403
            
        data = request.json
        new_role = data.get('role')
        if new_role not in ['Admin', 'Team Member', 'Member']:
...
def delete_team_member(user_id):
    ...
    try:
        ...
        client.table("tasks").update({"assigned_to": None}).eq("assigned_to", user_id).execute()
        
        # 2. Delete dependent data
        client.table("attendance").delete().eq("user_id", user_id).execute()
        ...
        # client.table("projects").update({"owner_id": current_user_id}).eq("owner_id", user_id).execute()
...
def get_projects():
    ...
    workspace_id = user.get('user_metadata', {}).get('workspace_id')
    
    if not user_id or not workspace_id: return jsonify({"error": "Unauthorized"}), 401
    ...
    try:
        ...
        if role == 'Admin':
            # Admin sees ALL projects in workspace
            res = client.table('projects').select('*').eq('workspace_id', workspace_id).order('created_at', desc=True).execute()
        else:
            # Non-Admins: See owned projects OR projects they are members of
            # 1. Get Project IDs where user is member
            memberships = client.table('project_members').select('project_id').eq('user_id', user_id).execute()
            member_pids = [str(m['project_id']) for m in memberships.data]
            
            # 2. Query projects where owner_id = user OR id in member_pids
            query = client.table('projects').select('*').eq('workspace_id', workspace_id).order('created_at', desc=True)
...
def get_project(project_id):
    try:
        ...
        try:
            ...
            m_res = client.table("project_members").select("role, user:user_id(id, full_name, avatar_url, email)").eq("project_id", project_id).execute()
...
def update_project(project_id):
    ...
    try:
        ...
        u_res = client.table('users').select('role').eq('id', user_id).single().execute()
        ...
        if updates.get('status') == 'Completed':
            try:
                ...
                # Get members
                pm_res = client.table('project_members').select('user_id').eq('project_id', project_id).execute()
...
def create_project():
    ...
    try:
        ...
        if members:
            ...
            if payload:
                try:
                    client.table("project_members").insert(payload).execute()
                except Exception as member_e:
                    print(f"Error adding members: {member_e}")
...
def get_stats():
    ...
    if request.args.get('debug_admin') == '1':
        print("DEBUG: Using Forced Admin User ID")
        user_id = '0c973aca-2f5a-4bee-ab9e-6a09d8f37a3c'
    ...
    try:
        ...
        r_res = verifier.table('users').select('role').eq('id', user_id).single().execute()
        if r_res.data:
            role = r_res.data.get('role', 'Team Member')
    except Exception as e:
    ...
    try:
        ...
        # Admin sees all projects in workspace
        p_res = supabase.table("projects").select("id, title").eq('workspace_id', workspace_id).execute()
...
def chat_ai():
    ...
    try:
        # Use simple counts with workspace filter
        u = supabase.table("users").select("id", count="exact").eq('workspace_id', workspace_id).execute()
        p = supabase.table("projects").select("id", count="exact").eq('workspace_id', workspace_id).execute()
        
        p_res = supabase.table("projects").select("id").eq('workspace_id', workspace_id).execute()
        project_ids = [str(proj["id"]) for proj in p_res.data]
        
        task_count = 0
        if project_ids:
            t = supabase.table("tasks").select("id").in_('project_id', project_ids).execute()
            task_count = len(t.data)
        context = f"Projects: {p.count}, Tasks: {task_count}"
    except Exception as e:
...
def get_attendance_today():
    ...
    try:
        res = client.table("attendance").select("*").eq("user_id", user_id).eq("date", today).single().execute()
        return jsonify(res.data)
    except Exception:
...
def punch_attendance():
    ...
    try:
        # Check existing
        existing = client.table("attendance").select("*").eq("user_id", user_id).eq("date", today).execute()
        ...
        if not existing.data:
            ...
            try:
                ...
                err_msg = str(e)
                if "missing" in err_msg and "location" in err_msg.lower():
            ...
            try:
                res = client.table("attendance").update(update_payload).eq("id", record["id"]).execute()
                return jsonify({"message": "Punched Out", "data": res.data[0], "type": "out"})
            except Exception as e:
                # Fallback if punch_out_location column is missing
                err_msg = str(e)
                if "missing" in err_msg and "punch_out_location" in err_msg.lower():
...
def get_attendance_history():
    ...
    try:
        query = client.table("attendance").select("*").eq("user_id", user_id).order("date", desc=True)
...
def update_profile():
    ...
    try:
        ...
        admin_client = get_supabase_admin()
        verifier = admin_client if admin_client else supabase
        res = verifier.table("users").update({"full_name": full_name}).eq("id", user_id).execute()
...
def invite_member():
    ...
    try:
        msg = MIMEMultipart()
        msg['From'] = f"ACCIO HUB <{sender_email}>"
        msg['To'] = email
